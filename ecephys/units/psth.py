...
def get_normed_data(data, normalize, window=None, binsize=None, verbose=True):
    ...
    if normalize == "all":
        ...
        norm_factors = np.reshape(max_rates, (max_rates.shape[0], 1)) * np.ones(
            (1, data.shape[1])
        )
        return np.divide(data, norm_factors, where=norm_factors != 0)
        ...
        baseline_mean = np.tile(
            np.mean(data[:, 0:n_baseline_idx], axis=1), (data.shape[1], 1)
        ).transpose()  # nunits x nbins, repeat mean across 1st column
        baseline_std = np.tile(
            np.std(data[:, 0:n_baseline_idx], axis=1), (data.shape[1], 1)
        ...
        return np.divide(data, baseline_mean, where=baseline_mean != 0, out=res)
    else:
        raise ValueError("Invalid value for `normalize` kwarg")


def get_evoked_firing_rates(
    spike_times, spike_clusters, unit_ids, events, plot_before, plot_after, time_bin
):
    """
    ...
    for indi, uniti in tqdm.tqdm(enumerate(unit_ids)):
        spikesi = np.atleast_1d(np.squeeze(spike_times[spike_clusters == uniti]))

        uniti_rates = []
        for E in events:
            window_spikes = spikesi[
                np.squeeze(
                    np.argwhere(
                        (spikesi >= E - plot_before) & (spikesi <= E + plot_after)
                    )
                )
            ]
