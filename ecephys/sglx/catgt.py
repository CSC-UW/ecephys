...
def get_catGT_command(catGT_path, wine_path=None, **kwargs):
    ...
    assert ("gtlist" in kwargs.keys()) or (
...
def _parse_gtlist(g_list, t_list):
    """Return {ga,tstart_a,tend_a}{...}{gn,tstart_end,tend_end}"""

    assert len(g_list) == len(t_list)

    g_i_array = np.array(
        [gstring.split("g")[1] for gstring in g_list], dtype=int
    )  # eg [0, 1]
    t_i_array = np.array(
        [tstring.split("t")[1] for tstring in t_list], dtype=int
    )  # eg [1, 2, 0, 1]
    ...
    for g in unique_g_idx:
        ...
        assert all(sorted(g_t_idx) == g_t_idx), (
            "Triggers should be sorted for each gate"
        )
        t_start = g_t_idx[0]
        t_end = g_t_idx[-1]
        assert len(g_t_idx) == len(range(t_start, t_end + 1)), (
            "There should be no gap in triggers for each gate"
        )
...
def run_catgt(raw_files, catgt_params, catgt_path, src_dir, tgt_dir, dry_run=True):
    ...
    gtlist = _parse_gtlist(raw_files.gate.values, raw_files.trigger.values)
