...
def get_windows(event_times, window, trim="end"):
    ...
    for i in range(nevents):
        ...
        if i < nevents - 1 and t_end > event_windows[i + 1, 0] and trim == "end":
            event_windows[i, 1] = event_windows[i + 1, 0]
        # Start of window overlaps with previous and we should trim the starts
        if i > 0 and t_start < event_windows[i - 1, 1] and trim == "start":
...
def get_locked_df(
    ...
    for event_idx, event_time in tqdm(enumerate(event_times)):
        ...
        # Add locked times
        data_df.loc[event_rows, time_rel_colname] = (
            data_df.loc[event_rows, time_colname] - event_times[event_idx]
        )
    ...
    data_df["event_window_duration"] = (
        data_df["event_window_end"] - data_df["event_window_start"]
    )

    if drop_out_of_window_datapoints:
        data_df = data_df.loc[~pd.isna(data_df["event_idx"])]
...
def get_event_firing_rates_df(
    ...
    ), print(locked_data_df.columns)

    # TODO
    # Check all events appear in frame
    if not len(locked_data_df.event_idx.unique()) == len(
        range(locked_data_df.event_idx.min(), locked_data_df.event_idx.max() + 1)
    ):
        import warnings
    ...
    event_rates = (
        ...
        .copy()
        .rename(columns={"t_rel": "event_num_spikes"})
    )
    ...
    filled_df = event_rates.groupby(level=1)[
        fb_filled_event_cols
    ].bfill()  # Backward-fill by event_idx
    filled_df = filled_df.groupby(level=1)[
        fb_filled_event_cols
    ].ffill()  # Further forward fill by event_idx
    event_rates.update(filled_df)

    event_rates["event_firing_rate"] = (
        event_rates["event_num_spikes"] / event_rates["event_window_duration"]
    )
