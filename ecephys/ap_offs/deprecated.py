...
class ImsegOffDetection():
    ...
    def __init__(self,
        ...
        if not type(chunk_size) == str:
            raise TypeError('chunk_size must be a string of form \'Xs\' where X is the duration of the chunk')
        else:
            self.chunk_size = chunk_size   
        self.wne_project = wet.get_wne_project('off_det_analysis')
        self.project_dir = self.wne_project.dir
        self.outdir = Path(self.project_dir /
                           self.subject / 
                           self.probe / 
                           f'{self.time_interval[0]}_{self.time_interval[1]}')
        
        self.si_recording, self.segments  = wet.get_sglx_subject(self.subject).get_si_recording(self.experiment,
                                                                                alias = self.alias,
                                                                                stream = 'ap',
                                                                                probe = self.probe,
        ...
        self.sample2time = self.sglx_project.get_sample2time(self.subject, 
                                                             self.experiment,
                                                             alias,
                                                             self.probe,
                                                             'sorting')
    ...
    def run(self):
        ...
        if self.project_dir / self.subject /self.probe not in Path.iterdir(self.project_dir/self.subject):
            Path.mkdir(self.project_dir / self.subject/ self.probe)
            
        print(f"Saving run in {self.outdir}")
        if self.outdir not in Path.iterdir(self.project_dir / self.subject / self.probe):
            Path.mkdir(self.outdir)
        #Timestampts and interval samples are saved to be reused, 
        frame_timestamps_fname = self.project_dir / self.subject / self.probe / 'timestamps.npy'
        ...
        '''
        self.si_recording = self.si_recording.frame_slice(start_frame=start_frame, end_frame=end_frame)
        ...
        utils.write_htsv(offs, self.outdir / offs_fname)
    ...
    '''
    def get_timestamps_interval_samples(self, frame_timestamps_fname, interval_samples_fname):
        if self.has_file(interval_samples_fname, self.outdir):
            print(f"Loading interval_samples file")
            #Either loads file containing recording timestamps from disk, or uses sampe2time to compute them
            #and then saves to file 
            frame_timestamps = self._get_frame_timestamps(self.segments, frame_timestamps_fname, self.project_dir /self.subject / self.probe)
            ...
            for i in range(len(frame_timestamps)):
                ...
                if (frame_timestamps[i] >= self.time_interval[0]
    ...
    def save_preprocessed_recording(self, recording):
        ...
        job_kwargs = dict(n_jobs=.5, chunk_duration=self.chunk_size, progress_bar=True)
        if Path(self.outdir / 'prepro.zarr').exists():
            import shutil
            shutil.rmtree(Path(self.outdir/ 'prepro.zarr'))
        recording.save_to_zarr(folder = Path(self.outdir / 'prepro'), format = 'zarr', **job_kwargs)        
    ...
    def wrap_xr(self, traces, times):
        ...
        if traces.shape[0] != len(self.chan_perm):
            raise Exception('No. of channels in traces array must be equal to chan_perm')
    ...
    def get_offs(self, traces_xr, thresholds = None):
        ...
        dask.config.set({'distributed.nanny.pre_spawn_environ.MALLOC_TRIM_THRESHOLD_': 0})
        ...
        if thresholds == None:
            if self.has_thresholds:
                thresholds_store = Path(self.project_dir / self.subject / self.probe / 'thresholds.zarr')
    ...
    def get_thresholds(self, traces):
        ...
        if self.save_thresholds:
            da.to_zarr(channel_thresholds, url = Path(self.project_dir /self.subject /self.probe / 'thresholds.zarr'), overwrite = True)
    ...
    def make_dfs(self, data, lbl_tmp_store):
        ...
        val_ixs = scipy.ndimage.value_indices(lbl_img, ignore_value=0) # {lbl: (row/time_indices, col/chan_indices)}
        del lbl_img

        print('reached')
        _lbls = np.sort(list(val_ixs.keys()))
        areas = pd.DataFrame([val_ixs[lbl][0].size for lbl in _lbls], columns=['area'], index=_lbls)
        start_frames = pd.DataFrame([val_ixs[lbl][0].min() for lbl in _lbls], columns=['start_frame'], index=_lbls)
        end_frames = pd.DataFrame([val_ixs[lbl][0].max() for lbl in _lbls], columns=['end_frame'], index=_lbls)
        strict_n_frames = pd.DataFrame([self._get_strict_n_frames(*val_ixs[lbl]) for lbl in _lbls], columns=['strict_n_frames'], index=_lbls)
        min_chan_ixs = pd.DataFrame([val_ixs[lbl][1].min() for lbl in _lbls], columns=['min_chan_ix'], index=_lbls)
        max_chan_ixs = pd.DataFrame([val_ixs[lbl][1].max() for lbl in _lbls], columns=['max_chan_ix'], index=_lbls)
        del val_ixs

    
        # df = pd.concat([cm_coords, pk_values, areas, start_frames, end_frames, strict_n_frames, min_chan_ixs, max_chan_ixs], axis=1).dropna()
        df = pd.concat([areas, start_frames, end_frames, strict_n_frames, min_chan_ixs, max_chan_ixs], axis=1).dropna()
        df['label'] = df.index
        ...
        sk_boundaries = da.from_array(skimage.util.apply_parallel(skimage.segmentation.find_boundaries, off_img, compute = True))
