...
def clean_binary_mask(
    ...
    if n_samples_connect is not None:
        ...
        off_mask.data = dask_image.ndmorph.binary_closing(
            off_mask.data, structure=struct, iterations=1
        )
    ...
    if n_samples_clean is not None:
        ...
        off_mask.data = dask_image.ndmorph.binary_opening(
            off_mask.data, structure=struct, iterations=1
        )
...
def get_offs_df(da, lbl_ixs):
    ...
    start_frames = pd.DataFrame(
        [lbl_ixs[lbl][0].min() for lbl in _lbls], columns=["start_frame"], index=_lbls
    )
    end_frames = pd.DataFrame(
        [lbl_ixs[lbl][0].max() + 1 for lbl in _lbls], columns=["end_frame"], index=_lbls
    )
    median_nframes = pd.DataFrame(
        [_get_median_nframes(*lbl_ixs[lbl]) for lbl in _lbls],
        columns=["median_nframes"],
        index=_lbls,
    )
    lo_chan_ix = pd.DataFrame(
        [lbl_ixs[lbl][1].min() for lbl in _lbls], columns=["lo_chan_idx"], index=_lbls
    )
    max_chan_idx = pd.DataFrame(
        [lbl_ixs[lbl][1].max() for lbl in _lbls], columns=["max_chan_idx"], index=_lbls
    )
    ...
    areas = pd.DataFrame(
        [lbl_ixs[lbl][0].size for lbl in _lbls], columns=["area"], index=_lbls
    )

    df = pd.concat(
        [areas, start_frames, end_frames, median_nframes, lo_chan_ix, max_chan_idx],
        axis=1,
    ).dropna()
    df["label"] = df.index
    ...
    df["end_time"] = (
        t[df["end_frame"].values - 1] + 1 / da.attrs["fs"]
    )  # In case end_frame is on a gap
    ...
    def _assign_convexity_metrics(df, lbl_ixs):
        ...
        convex_area = df.apply(lambda row: _get_row_area(row), axis=1)
...
def detect_ap_offs(da, thresholds, opts=None):
    ...
    if missing_keys:
        ...
        warnings.warn(
            f"Following keys might be missing from `opts` kwarg: {missing_keys}"
        )
    ...
    lbl_ixs = scipy.ndimage.value_indices(
        np.array(lbl_da.data), ignore_value=0
