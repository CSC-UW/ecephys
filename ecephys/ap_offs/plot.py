...
def add_ap_off_overlay(
    da: xr.DataArray, lbl_ixs: dict, offs_df: pd.DataFrame, ax, xlim, **plot_kwargs
):
    offs_sel = offs_df[
        (offs_df.start_time.between(*xlim)) & (offs_df.end_time.between(*xlim))
    ]
...
def add_mean_ap_off_overlay(
    offs_df: pd.DataFrame, ax, xlim: tuple[float], ax_ylim: tuple[float], **plot_kwargs
):
    offs_sel = offs_df[
        (offs_df.start_time.between(*xlim)) & (offs_df.end_time.between(*xlim))
    ]
