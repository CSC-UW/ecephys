...
def prepare_lfp(
    lfp: xr.DataArray, new_times: np.ndarray[float], new_fs: float
) -> xr.DataArray:
    ...
    assert np.allclose(np.diff(new_times), 1 / new_fs), (
        "New times do not match new sampling rate."
    )
    f = scipy.interpolate.interp1d(
        lfp.time, lfp.T, kind="cubic", bounds_error=False, fill_value=0.0
    )
    ...
    for i in range(data.shape[0]):
        data[i] = ecephys.utils.clip_outliers(data[i], method="mad", n_devs=24)
...
def prepare_emg(
    emg: xr.DataArray, new_times: np.ndarray[float], new_fs: float
) -> xr.DataArray:
    # We cannot resample using scipy.interp1d without removing duplicate timestamps
    ...
    f = scipy.interpolate.interp1d(
        emg.time, emg.T, kind="cubic", bounds_error=False, fill_value=0.0
    )
    data = f(new_times)

    # Zero-fill any problematic values, remove outliers that would affect scaling
    data = np.nan_to_num(data, nan=0.0, posinf=0.0, neginf=0.0)
    data = ecephys.utils.clip_outliers(data, method="mad", n_devs=6)
...
def write_edf_for_visbrain(
    ...
    assert np.allclose(lfp_t0, emg_t0, atol=0.100), (
