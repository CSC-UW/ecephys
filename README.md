## ecephys
[![Build Status](https://travis-ci.org/uwescience/ecephys.svg?branch=master)](https://travis-ci.org/uwescience/ecephys)
...
`ecephys/ecephys.py`. The code provided in this file is intentionally rather
...
We place the module code in a file called `ecephys.py` in directory called
`ecephys`. This structure is a bit confusing at first, but it is a simple way
to create a structure where when we type `import ecephys as sb` in an
interactive Python session, the classes and functions defined inside of the
`ecephys.py` file are available in the `sb` namespace. For this to work, we
...
In the case of the ecephys module, the main classes defined at the bottom of
...
Either way, you can create a `ecephys/data` folder in which you can
...
(typically, something like our `ecephys/tests/test_ecephys.py`). Within each
...
have one such test in `ecephys/tests/test_ecephys.py` called
...
the top-level directory of the repository (e.g. `/Users/arokem/code/ecephys`),
and type:
...
will see a message such as:
    ...
    ecephys/tests/test_ecephys.py .F...
    ...
    ecephys/tests/test_ecephys.py:49: AssertionError
...
To document `ecephys` we use the [sphinx documentation
...
![RTD conf](https://github.com/uwescience/ecephys/blob/master/doc/_static/RTD-advanced-conf.png)

 http://ecephys.readthedocs.org/en/latest/
...
A `ecephys/version.py` contains all of the information needed for the
installation and for setting up the [PyPI
page](https://pypi.python.org/pypi/ecephys) for the software. This
...
`ecephys` against Python 2.7, Python 3.3 and Python 3.4). It includes steps
that need to be taken before installation of the software. For example,
installation of the software dependencies. For `ecephys`, we use the
...
[here](https://github.com/uwescience/ecephys/blob/master/scripts/Figure1.ipynb).
...
button on the front page of the "ecephys" repository.
...
`ecephys/ecephys.py` to be called `smallish/smallish.py`

	git mv ecephys smallish
	git mv smallish/ecephys.py smallish/smallish.py
	git mv smallish/tests/test_ecephys.py smallish/tests/test_smallish.py

Make a commit recording these changes. Something like:

	git commit -a -m"Moved names from `ecephys` to `smallish`"

You will probably want to remove all the example data:

	git rm smallish/data/*
	git commit -a -m"Removed example `ecephys` data"

Possibly, you will want to add some of your own data in there.

You will want to edit a few more places that still have `ecephys` in them. Type
the following to see where all these files are:

	git grep ecephys

You can replace `ecephys` for `smallish` quickly with:

	git grep -l 'ecephys' | xargs sed -i 's/ecephys/smallish/g'
...
Edit all the mentions of `ecephys` in the `ecephys/__init__.py` file, and in
the `ecephys/version.py` file as well.
